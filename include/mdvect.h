...
//typedef vector<lreal> dvector;

//////////////////////////////////////////////////////////////////////////////

template<class _T> lreal lVLen(_T _v){
	lreal _sum = 0;
...
template<class _T> real fVLen(_T _v){
	real _sum = 0;
	for(unsigned int i=0; i<_v.size(); i++)
	    _sum += _v[i]*_v[i];
	return sqrt(_sum);
...
template<class _T, class _Tu> TVector<lreal> lVShift(_T _v, _Tu  _scale){
	TVector<lreal> _t(_v.size());
	for( unsigned int i=0; i<_v.size(); i++)
	    _t[i] = lreal(_v[i] + _scale);
	return _t;
