...
private:
  ...
  bool isnrow(unsigned int idx){
    ...
    if(inm){
#ifdef __GET_MESSAGE
      std::cerr <<matrix_name<<" row index out of range - "<<idx<<std::endl<<" EXIT "<<EXIT<<std::endl;
#endif
      exit(EXIT);
    }
    return inm;
  ...
  bool isncol(unsigned int idx){
    ...
    if(inm){
#ifdef __GET_MESSAGE
      std::cerr <<matrix_name<<" column index out of range - "<<idx<<std::endl<<" - EXIT "<<EXIT<<std::endl;
#endif
      exit(EXIT);
    }
    return inm;
...
public:
  ...
  TMatrix(const char* s, unsigned int r, unsigned int c){
    name(s);
    init();
    set(r,c);
  }
  ...
  void write_file(const char* f){
    write_file(f, i_precision);
  }

  // Wed Aug  8 09:13:49 MDT 2012
  // deprecated
  ...
  void write_file(const char* f, int _p){
  ...
  // Sun Oct 21 12:20:51 MDT 2012
  // bug fixed when deleting the last row
  void remove_row(unsigned int u){
    if(u<rows()-1) m_data[u]=m_data[rows()-1];
    i_rows--;
    reset(i_rows,cols());
  }
  ...
  // Sun Oct 21 12:20:51 MDT 2012
  // modified when deleting the last column
  void del_col(unsigned int c){
    for(unsigned int r=0; r<rows(); r++){
      if(c<cols()-1) m_data[r][c]=m_data[r][cols()-1];
      m_data[r].remove();
    }
    i_cols--;
  }
  ...
  //void operator= (const TMatrix<T>& m){
    if(this != &m){
  ...
  // use the next operator instead this
  ...
  friend std::ostream& operator << (std::ostream& o, TMatrix<T>& m){
    o <<m.matrix_name<<" = {"<<m.i_rows<<"x"<<m.i_cols<<"} ="<<std::endl;
