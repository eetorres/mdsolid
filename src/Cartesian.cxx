...
void Ca_X_Axis::draw(){
    ...
    if (damage()&(FL_DAMAGE_ALL|CA_DAMAGE_ALL)){
        ...
        int a, b, l1=0, l2=0, m1=0, m2=0, l=0 ,_y=0,_w,_h; //temporary coordinates for ticks
        double _pos,_x;
...
//        draw_label();
    if (damage()&(FL_DAMAGE_ALL|CA_DAMAGE_ALL)){
        ...
        if(!valid_) return;
        fl_font(label_font_face_,label_font_size_);
        int a, b, l1=0, l2=0, m1=0, m2=0, l=0,_x=0,_w,_h; //temporary coordinates for ticks
        double _pos,_y;
        fl_clip(x()+Fl::box_dx(box()),y()+Fl::box_dy(box()),w()-Fl::box_dw(box()),h()-Fl::box_dh(box()));
		fl_color(axis_color_);
...
void Ca_PolyLine::draw(){
	...
	while(temp){
		...
		if(((int)temp->line_style != style)||((int)temp->color!=c)||((int)temp->line_width!=size)){
			fl_end_line();
			c=temp->color;
